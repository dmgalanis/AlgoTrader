...
class AlgoEngine:
    ...
    async def reload_settings(self):
        ...
        # If daily strategy is enabled but candles haven't been loaded yet
        # (e.g. strategy was enabled after startup), load them now.
        if sd.get("enabled") and not self.daily_candles:
            log.info("📅 Daily strategy enabled — loading daily candles...")
            await asyncio.to_thread(self._load_daily_candles)
        # Also load candles for any new symbols added to the daily watchlist
        elif sd.get("enabled") and self.client is not None:
            missing = [s for s in sd["watchlist"] if s not in self.daily_candles]
            if missing:
                log.info("📅 Loading daily candles for new symbols: %s", ", ".join(missing))
                for sym in missing:
                    try:
                        self.daily_candles[sym] = await asyncio.to_thread(
                            self.client.get_daily_candles, sym, 300)
                    except Exception as e:
