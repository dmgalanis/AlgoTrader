...
class TastyTradeClient:
    ...
    def place_limit(self, symbol: str, quantity: int, limit_price: float) -> str:
        ...
        log.info("  📋 place_limit payload: %s", order)
        resp = requests.post(
            f"{self.base}/accounts/{self.account_number}/orders",
            headers=self._h(), json=order, timeout=10)
        log.info("  📋 place_limit response %d: %s", resp.status_code, resp.text)
        resp.raise_for_status()
        return str(resp.json()["data"]["order"]["id"])
...
async def execute_entry(client, symbol, intended_price, signal,
    ...
    try:
        ...
        try:
            ...
            log.error("  ❌ Stop placement failed for %s: %s — POSITION TRACKED, place stop manually", symbol, e)

        # ── Take-profit tiers ─────────────────────────────────────────────
        initial_risk = fill_price * stop_pct
        tp_placed = []
        log.info("  🔍 TP tiers config: %s", strategy_cfg.get("take_profit_tiers", "MISSING"))
        for tier in strategy_cfg["take_profit_tiers"]:
            ...
            try:
                ...
                log.error("  ❌ TP %sR placement failed for %s: %s", tier["r_multiple"], symbol, e)
