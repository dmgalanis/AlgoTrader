...
class TastyTradeClient:
    ...
    def place_oco(self, symbol: str, quantity: int,
                  stop_price: float, limit_price: float) -> str:
        """
        Place a single OCO (One-Cancels-Other) complex order combining stop-loss
        and take-profit. TastyTrade rejects two separate closing orders on the
        same position — OCO is the required approach.
        Returns the complex order ID.
        """
        leg = {"instrument-type": "Equity", "symbol": symbol,
               "quantity": quantity, "action": "Sell to Close"}
        payload = {
            "type": "OCO",
            "orders": [
        ...
        if enabled_tiers:
            ...
            try:
                oco_id = await asyncio.to_thread(
                    client.place_oco, symbol, tp_shares, actual_stop, tp_price)
                log.info("  🛑🎯 OCO placed — stop=$%.2f  TP %sR=$%.2f  qty=%d (id=%s)",
                         actual_stop, tier["r_multiple"], tp_price, tp_shares, oco_id)
