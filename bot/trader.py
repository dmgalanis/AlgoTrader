...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(os.path.join(os.path.dirname(os.path.abspath(__file__)), "logs", "trader.log"), mode="a"),
    ],
)
log = logging.getLogger(__name__)

# ── Paths / constants ──────────────────────────────────────────────────────────
ROOT          = os.path.dirname(os.path.abspath(__file__))   # /app when run from repo root
...
class _StatusHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        # Safety check — prevent live trading from being enabled without
        # explicit confirmation flag to avoid accidental live orders
        exec_cfg = new_settings.get("execution", {})
        algo_on  = exec_cfg.get("algo_enabled", False)
        paper_on = exec_cfg.get("paper_trading", True)
        if algo_on and not paper_on:
            confirmed = new_settings.get("confirm_live_trading", False)
            if not confirmed:
                log.warning("⚠️  Live trading enable blocked — confirm_live_trading not set")
                self._send_json(403, {
                    "error": "Live trading requires confirm_live_trading=true in payload",
                    "blocked": True,
                })
                return
...
def load_settings_safe() -> dict:
    """
    Load settings but force safe defaults on startup.
    algo_enabled is always False on startup — must be explicitly enabled
    via the dashboard after the bot is running. This prevents live trades
    firing immediately on container restart before the operator is watching.
    """
    cfg = load_settings()
    if cfg["execution"].get("algo_enabled"):
        log.warning("⚠️  algo_enabled=true in settings.json — forcing FALSE on startup.")
        log.warning("⚠️  Enable algo trading explicitly via the dashboard when ready.")
...
class AlgoEngine:
    def __init__(self):
        self.settings = load_settings_safe()   # safe startup — algo disabled until explicit enable
    ...
    async def reload_settings(self):
        self.settings = load_settings_safe()
    ...
    async def _evaluate(self, symbol: str, price: float, ts: float):
        try:
            ...
            # Read algo_enabled fresh from disk — never trust cached value for
            # the execution gate since a stale cache is what caused live trades
            try:
                with open(SETTINGS_PATH) as _sf:
                    algo_on = json.load(_sf)["execution"]["algo_enabled"]
            except Exception:
                algo_on = False   # safe default if file unreadable
    ...
    async def run(self):
        log.info("═" * 60)
        log.info("  TastyTrade Multi-Strategy Trader  v3.1")
        log.info("═" * 60)
