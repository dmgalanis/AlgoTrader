"""
TastyTrade Multi-Strategy Auto-Trader v4.5
==========================================
- Alpaca SIP websocket for market data
- 15-min EMA Cloud (S1) and Daily EMA Pullback (S2) strategies
- Paper trading safety locks — algo_enabled never persisted to file
- Status HTTP server with CORS for dashboard polling
- File-based state persistence (no Redis)
"""
...
STATUS_PORT        = int(os.environ.get("PORT", 8080))
SIGNAL_LOG_MAX     = 100
    ...
    def write(self):
        try:
            ...
            with open(STATUS_PATH, "w") as f:
                json.dump(self.to_dict(), f, cls=_Enc)
...
class _StatusHandler(BaseHTTPRequestHandler):
    ...
    def _check_secret(self) -> bool:
        ...
        if not match:
            log.warning("⚠️  Secret mismatch — provided_len=%d expected_len=%d provided='%s'",
                        len(provided), len(secret), provided[:4] + "***" if provided else "(empty)")
        return match
        ...
        if self.path in ("/status.json", "/"):
            ...
            class _Enc(json.JSONEncoder):
                def default(self, o):
                    ...
                    if isinstance(o, np.ndarray):     return o.tolist()
                    return super().default(o)
            data = json.dumps(self.tracker.to_dict(), cls=_Enc).encode()
    ...
    def do_POST(self):
        ...
        # Extract algo_enabled and confirm flag before saving
        # algo_enabled is NEVER written to file — only applied to memory
        algo_requested  = new_settings.get("execution", {}).get("algo_enabled", False)
        paper_requested = new_settings.get("execution", {}).get("paper_trading", True)
        confirmed       = new_settings.pop("confirm_live_trading", False)

        # Always save with algo_enabled=False so file is always safe
        new_settings["execution"]["algo_enabled"] = False

        # Block live trading without explicit confirmation
        if algo_requested and not paper_requested and not confirmed:
            log.warning("⚠️  Live trading blocked — confirm_live_trading not set")
            self._send_json(403, {"error": "Live trading requires confirmation", "blocked": True})
            return

        # Write safe settings to file
        try:
        ...
        # Apply algo_enabled to engine memory only (never persisted to file).
        # Must always apply the value — including False — so the dashboard OFF
        # toggle actually stops trading. The original code only entered this
        # branch when algo_requested was True, so turning off did nothing.
        if not algo_requested:
            self.tracker.algo_enabled = False
            log.info("⚙️  algo_enabled set to False in memory — trading halted")
        elif paper_requested or confirmed:
            self.tracker.algo_enabled = True
            log.info("⚙️  algo_enabled set to True in memory (not saved to file)")
...
def start_status_server(tracker: StatusTracker):
    ...
    t.start()
    log.info("🌐 Status server on http://0.0.0.0:%d  (PORT env=%s, secret: %s, secret_len: %d)",
             STATUS_PORT, os.environ.get("PORT", "not set"), "set ✅" if secret else "not set ⚠️", len(secret))
    return server
...
def load_settings() -> dict:
    with open(SETTINGS_PATH) as f:
        cfg = json.load(f)
    # SAFETY: algo_enabled is NEVER loaded from file — it must be explicitly
    # enabled via the dashboard after the bot is confirmed running.
    # This prevents live trades firing automatically on any restart or redeploy.
    cfg["execution"]["algo_enabled"] = False
    return cfg
...
def load_state() -> dict:
    """Load state from local JSON file — simple and reliable."""
    try:
        if os.path.exists(STATE_PATH):
            ...
            if isinstance(parsed, dict):
                ...
                state.update(parsed)
                log.info("✅ State loaded from %s", STATE_PATH)
                return state
    except Exception as e:
        log.warning("⚠️  Could not load state: %s — using default", e)
...
def save_state(state: dict):
    ...
    try:
        ...
        log.warning("⚠️  Could not save state: %s", e)
...
class TastyTradeClient:
    ...
    def place_market_buy(self, symbol: str, quantity: int) -> str:
        if self.paper:
            raise RuntimeError(f"SAFETY BLOCK: place_market_buy in paper mode for {symbol}")
        order = {
    ...
    def poll_fill(self, order_id: str, requested_qty: int = 0) -> Optional[dict]:
        """
        Poll until the order is Filled, Cancelled, Rejected, or Expired.
        TastyTrade market orders typically go: Received -> Routed -> Live -> Filled.
        requested_qty is used as a fallback when TastyTrade marks an order Filled
        before populating the fill details (common on live accounts).
        """
        ...
        while time.time() < deadline:
            try:
                ...
                if status != last_status:
                    ...
                    last_status = status
                if status == "Filled":
                    # Try legs/fills first (most detailed)
                    legs = order.get("legs", [{}])
                    ...
                    # Try top-level filled-quantity / average-fill-price
                    filled_qty = int(order.get("filled-quantity") or 0)
                    avg_price  = float(order.get("average-fill-price") or 0)
                    if filled_qty > 0:
                        return {"filled_quantity": filled_qty, "average_fill_price": avg_price}
                    # TastyTrade marks Filled before populating details on live accounts.
                    # Return a sentinel using requested_qty so stop/TP can still be placed.
                    # Price=0 signals execute_entry to use intended_price as fallback.
                    log.info("  🔄 Order %s Filled — details pending, using requested qty", order_id)
                    return {"filled_quantity": requested_qty, "average_fill_price": 0.0}
        ...
        # Timeout — do one final check before giving up
        try:
            order = self.get_order(order_id)
            status = order.get("status", "")
            log.warning("  ⚠️  Fill timeout for order %s — final status: %s", order_id, status)
            if status == "Filled":
                legs = order.get("legs", [{}])
                fills = legs[0].get("fills", []) if legs else []
                if fills:
                ...
                if filled_qty > 0:
                    return {"filled_quantity": filled_qty, "average_fill_price": avg_price}
                # Status is Filled but TastyTrade hasn't populated qty even after
                # full timeout — return requested_qty so stop/TP can still be placed.
                log.warning("  ⚠️  Order %s Filled but qty still 0 after timeout — using requested qty", order_id)
                return {"filled_quantity": requested_qty, "average_fill_price": 0.0}
        except Exception as e:
            log.warning("  ⚠️  Final poll error for %s: %s", order_id, e)
        return None

    def place_stop(self, symbol: str, quantity: int, stop_price: float) -> str:
        """Place a standalone stop-loss (used when there are no TP tiers enabled)."""
        order = {
        ...
        resp = requests.post(
            ...
            headers=self._h(), json=order, timeout=10)
        if not resp.ok:
            log.error("  ❌ place_stop error %d: %s", resp.status_code, resp.text)
        resp.raise_for_status()
        return str(resp.json()["data"]["order"]["id"])

    def place_oco(self, symbol: str, stop_qty: int, tp_qty: int,
                  stop_price: float, limit_price: float) -> str:
        """
        Place an OCO (One-Cancels-Other) complex order.
        stop_qty  — full position size (all shares protected by the stop)
        tp_qty    — partial size for the limit/TP (e.g. 30% of position)
        When the TP fires, the stop cancels. position_monitor then detects the
        reduced position and places a new stop for the remaining shares.
        """
        stop_leg  = {"instrument-type": "Equity", "symbol": symbol,
                     "quantity": stop_qty, "action": "Sell to Close"}
        tp_leg    = {"instrument-type": "Equity", "symbol": symbol,
                     "quantity": tp_qty,  "action": "Sell to Close"}
        payload = {
            "type": "OCO",
            "orders": [
                {
        ...
        resp = requests.post(
            f"{self.base}/accounts/{self.account_number}/complex-orders",
            headers=self._h(), json=payload, timeout=10)
        if not resp.ok:
            log.error("  ❌ place_oco error %d: %s", resp.status_code, resp.text)
        resp.raise_for_status()
        data = resp.json()["data"]
        oco_id = str(data["complex-order"]["id"])
        return oco_id

    def place_limit(self, symbol: str, quantity: int, limit_price: float) -> str:
        """Standalone limit order — kept for compatibility but not used for TP."""
        order = {
            "time-in-force": "GTC", "order-type": "Limit",
            "price": str(round(limit_price, 2)),
            "price-effect": "Credit",
        ...
        resp = requests.post(
            ...
            headers=self._h(), json=order, timeout=10)
        if not resp.ok:
            log.error("  ❌ place_limit error %d: %s", resp.status_code, resp.text)
        resp.raise_for_status()
        return str(resp.json()["data"]["order"]["id"])

    def get_daily_candles(self, symbol: str, count: int = 100) -> pd.DataFrame:
        # TastyTrade's candle endpoint returns 400/429 errors unreliably.
        # Alpaca already works perfectly for 15-min bars — use it for daily too.
        ...
        log.info("  📊 Alpaca daily candles for %s — %d bars", symbol, len(df))
...
def check_signal_daily(daily_df, price, cfg, status: StatusTracker, symbol) -> Optional[str]:
    ...
    if len(daily_df) < period + 1:
        return None
    df = daily_df.copy()
    # Inject the live intraday price as today's close so the EMA reflects
    # the current session.  Without this the EMA is always one day stale
    # and the dist_pct threshold check is computed against yesterday's close.
    ...
    # Price at or below EMA — not a valid pullback setup (we want price just
    # above the EMA, pulling back into it from above).  Record to the signal
    # log so the dashboard always shows S2 activity, then bail out.
    if price <= ema_val:
        status.record_signal(symbol, "S2-Daily", "pullback", price,
                             ema_val, dist_pct, False)
        return None
...
async def execute_entry(client, symbol, intended_price, signal,
    ...
    # HARD SAFETY GATE: client.paper is set once at startup from settings and
    # never changes. Do NOT re-read settings.json here — a stale or unwritten
    # file would silently block all live trades and leave state un-updated.
    if client.paper:
        log.info("  [PAPER] No order placed — client initialised in paper mode")
        return False
    ...
    try:
        ...
        fill = await asyncio.to_thread(client.poll_fill, buy_id, shares)
        if not fill:
            # poll_fill only returns None for Cancelled/Rejected/Expired orders.
            # Record position at intended price so we don't re-enter, but no stop/TP.
            log.error("  ❌ [%s] Order cancelled/rejected — recording at intended price.", symbol)
            stop_pct2 = strategy_cfg["risk"]["stop_loss_pct"] / 100
            state[position_key][symbol] = {
                "entry_price": intended_price, "stop_price": round(intended_price * (1 - stop_pct2), 2),
                "shares": shares, "signal": signal, "entry_time": datetime.now(tz).strftime("%H:%M:%S"),
                "buy_order_id": buy_id, "stop_order_id": None, "tp_tiers": [], "fill_unconfirmed": True,
            }
        ...
        # TastyTrade sometimes marks an order Filled before populating the fill
        # price in the API response (average_fill_price returns 0.0).  Poll for
        # up to ~4 seconds in 0.5s intervals to retrieve the actual price before
        # falling back to intended_price — avoiding miscalculated stops/TPs.
        if fill_price == 0.0:
            log.info("  ⏳ Fill price not yet populated — polling for actual price...")
            for attempt in range(8):
                time.sleep(0.5)
                try:
                    order = client.get_order(buy_id)
                    legs  = order.get("legs", [{}])
                    fills = legs[0].get("fills", []) if legs else []
                    if fills:
                        total_qty   = sum(float(f.get("fill-quantity", 0)) for f in fills)
                        total_cost  = sum(float(f.get("fill-quantity", 0)) * float(f.get("fill-price", 0)) for f in fills)
                        if total_qty > 0:
                            fill_price = round(total_cost / total_qty, 4)
                            log.info("  ✅ Actual fill price retrieved on attempt %d: $%.4f", attempt + 1, fill_price)
                            break
                    avg = float(order.get("average-fill-price") or 0)
                    if avg > 0:
                        fill_price = avg
                        log.info("  ✅ Actual fill price (top-level) on attempt %d: $%.4f", attempt + 1, fill_price)
                        break
                except Exception as e:
                    log.warning("  ⚠️  Fill price retry %d error: %s", attempt + 1, e)
            else:
                log.warning("  ⚠️  Fill price unavailable after retries — falling back to intended $%.4f", intended_price)

        fill_price = fill_price or intended_price  # final safety fallback
        if fill_price == intended_price:
            log.info("  ℹ️  Using intended price $%.4f for stop/TP calc", intended_price)
        ...
        # ── Record position IMMEDIATELY after confirmed fill ──────────────
        # Must happen before place_stop / place_limit. If those calls throw,
        # the outer except would swallow the error and return False without
        # ever writing state — leaving the bot blind to an open position and
        # causing it to re-enter on the next tick.
        log.info("  💾 [%s] Writing position to state...", symbol)
        state[position_key][symbol] = {
        ...
        # TastyTrade does not allow two separate closing orders on the same
        # position. Stop and TP must be submitted as a single OCO complex order.
        # If no TP tiers are enabled, fall back to a standalone stop order.
        initial_risk = fill_price * stop_pct
        enabled_tiers = [t for t in strategy_cfg["take_profit_tiers"] if t["enabled"]]

        tp_placed = []
        if enabled_tiers:
            # Use the first enabled tier for the OCO TP price.
            # Additional tiers (partial exits) are not supported in a single OCO;
            # place them as separate OCOs on the remaining quantity after first TP fills.
            tier = enabled_tiers[0]
            tp_price  = round(fill_price + initial_risk * tier["r_multiple"], 2)
            tp_shares = max(1, int(filled_qty * (tier["sell_pct"] / 100)))
            try:
                oco_id = await asyncio.to_thread(
                    client.place_oco, symbol, filled_qty, tp_shares, actual_stop, tp_price)
                log.info("  🛑🎯 OCO placed — stop=%d×$%.2f  TP %sR=%d×$%.2f (id=%s)",
                         filled_qty, actual_stop, tier["r_multiple"], tp_shares, tp_price, oco_id)
                state[position_key][symbol]["stop_order_id"] = oco_id
                tp_placed.append({"r": tier["r_multiple"], "price": tp_price,
                                  "shares": tp_shares, "order_id": oco_id})
            except Exception as e:
                log.error("  ❌ OCO placement failed for %s: %s — placing standalone stop", symbol, e)
                try:
                    sl_id = await asyncio.to_thread(client.place_stop, symbol, filled_qty, actual_stop)
                    log.info("  🛑 Fallback stop-loss @ $%.2f (id=%s)", actual_stop, sl_id)
                    state[position_key][symbol]["stop_order_id"] = sl_id
                except Exception as e2:
                    log.error("  ❌ Stop placement also failed for %s: %s — PLACE MANUALLY", symbol, e2)
        else:
            # No TP tiers enabled — standalone stop only
            try:
                sl_id = await asyncio.to_thread(client.place_stop, symbol, filled_qty, actual_stop)
                log.info("  🛑 Stop-loss @ $%.2f (id=%s)", actual_stop, sl_id)
                state[position_key][symbol]["stop_order_id"] = sl_id
            except Exception as e:
                log.error("  ❌ Stop placement failed for %s: %s — PLACE MANUALLY", symbol, e)

        state[position_key][symbol]["tp_tiers"] = tp_placed
        save_state(state)
...
def fetch_alpaca_15min_bars(symbol: str, api_key: str, secret_key: str,
    ...
    while len(bars) < count:
        # Fetch newest bars first (sort=desc) + start date to ensure
        # multi-day history is returned, not just today's session.
...
class AlgoEngine:
    ...
    async def reload_settings(self):
        sd_was_enabled = self.settings.get("strategy_daily", {}).get("enabled", False)
        self.settings = load_settings()
        ...
        log.info("⚙️  Settings reloaded — S1=%s (limit=%d) S2=%s (limit=%d) candles=%s",
                 s15.get("enabled"), s15.get("daily_trade_limit"),
                 sd.get("enabled"),  sd.get("daily_trade_limit"),
                 list(self.daily_candles.keys()) or "none")
        # NOTE: algo_enabled is intentionally NOT updated here — it lives
        # only in memory and is set via /settings POST with confirmation.
        # Reloading from file would always reset it to False.
        # If S2 was just toggled on, daily candles won't exist yet — load them now.
        sd_now_enabled = sd.get("enabled", False)
        if sd_now_enabled and not sd_was_enabled:
            log.info("⚙️  S2 toggled on — loading daily candles now...")
            self._daily_candles_date = None   # clear guard so _load_daily_candles doesn't skip
    ...
    def _reset_daily_if_needed(self):
        if not isinstance(self.state, dict):
            log.error("❌ self.state is %s — reinitialising", type(self.state))
    ...
    def reconcile_positions(self, periodic: bool = False):
        """
        Sync self.state positions against live TastyTrade positions.
        - Removes fully closed positions and frees the daily trade slot.
        - Detects partial TP fills (live qty < state qty) and places a new
          standalone stop for the remaining shares at the original stop price.
        periodic=True suppresses startup log noise and only saves on change.
        """
        if not periodic:
            log.info("🔍 Reconciling positions...")
        try:
            live      = self.client.get_positions()
            live_qtys = {p["symbol"]: int(float(p.get("quantity", 0)))
                         for p in live if float(p.get("quantity", 0)) > 0}
            syms    = set(live_qtys.keys())
            changed = False
            now     = time.time()

            for pos_key, counter_key in (("s15_positions", "s15_trades_today"),
                                         ("sd_positions",  "sd_trades_today")):
                for s in list(self.state.get(pos_key, {}).keys()):
                    pos      = self.state[pos_key][s]
                    entry_ts = pos.get("recorded_at", 0)

                    if s not in syms:
                        # Position fully closed
                        if periodic and (now - entry_ts) < 90:
                            log.info("  ⏳ %s not yet in API — skipping eviction (age %.0fs)", s, now - entry_ts)
                            continue
                        log.info("  📉 %s closed — removing from state, freeing trade slot.", s)
                        del self.state[pos_key][s]
                        self.state[counter_key] = max(0, self.state.get(counter_key, 0) - 1)
                        changed = True

                    else:
                        # Position still open — check for partial TP fill
                        live_qty  = live_qtys[s]
                        state_qty = pos.get("shares", 0)
                        if (live_qty < state_qty
                                and not pos.get("remainder_stop_placed", False)
                                and (now - entry_ts) >= 90):
                            remaining = live_qty
                            stop_px   = pos["stop_price"]
                            log.info("  🔄 %s partial TP detected — %d→%d shares remaining. "
                                     "Placing new stop @ $%.2f", s, state_qty, remaining, stop_px)
                            try:
                                sl_id = self.client.place_stop(s, remaining, stop_px)
                                log.info("  🛑 Replacement stop placed for %s × %d @ $%.2f (id=%s)",
                                         s, remaining, stop_px, sl_id)
                                pos["shares"]                = remaining
                                pos["stop_order_id"]         = sl_id
                                pos["remainder_stop_placed"] = True
                                changed = True
                            except Exception as e:
                                log.error("  ❌ Failed to place replacement stop for %s: %s", s, e)
    ...
    def _load_daily_candles(self):
        ...
        for sym in cfg["watchlist"]:
            try:
                self.daily_candles[sym] = self.client.get_daily_candles(sym, count=100)
                loaded += 1
                time.sleep(0.5)   # avoid 429 rate limit — TastyTrade allows ~2 req/s
            except Exception as e:
                log.warning("  ⚠️  Daily candles failed for %s: %s", sym, e)
                time.sleep(1.0)   # back off longer after an error
        self._daily_candles_date = today
        log.info("✅ Daily candles loaded — %d/%d symbols OK.", loaded, len(cfg["watchlist"]))
    ...
    async def _evaluate(self, symbol: str, price: float, ts: float):
        try:
          async with self._lock:
            ...
            # Read from status tracker memory — algo_enabled is never persisted
            # to file so reading the file would always return False
            algo_on = self.status.algo_enabled
                ...
                if not df.empty:
                    ...
                    if sig:
                        ...
                        if entry_cooled and algo_on and in_market:
                            already_in_any = (symbol in self.state.get("s15_positions", {}) or
                                              symbol in self.state.get("sd_positions",  {}))
                            trades_today   = self.state.get("s15_trades_today", 0)
                            log.info("🔍 [S1] %s guard — in_pos=%s trades=%d/%d open=%s",
                                     symbol, already_in_any, trades_today, s15["daily_trade_limit"],
                                     list(self.state.get("s15_positions", {}).keys()))
                            if (not already_in_any and trades_today < s15["daily_trade_limit"]):
                                ...
                                # Blocked by limit or position — still advance cooldown
                                # so the guard doesn't flood logs every tick
                                self._last_signal["s15"][symbol] = ts
                        elif not (algo_on and in_market) and entry_cooled:
                            self._last_signal["s15"][symbol] = ts
                ...
                if daily_df is not None and not daily_df.empty:
                    ...
                    if sig:
                        ...
                        if entry_cooled and algo_on and in_market:
                            already_in_any = (symbol in self.state.get("s15_positions", {}) or
                                              symbol in self.state.get("sd_positions",  {}))
                            trades_today = self.state.get("sd_trades_today", 0)
                            log.info("🔍 [S2] %s guard — in_pos=%s trades=%d/%d open=%s",
                                     symbol, already_in_any, trades_today, sd["daily_trade_limit"],
                                     list(self.state.get("sd_positions", {}).keys()))
                            if (not already_in_any and
                                    trades_today < sd["daily_trade_limit"]):
    ...
    async def run(self):
        log.info("═" * 60)
        log.info("  TastyTrade Multi-Strategy Trader  v4.5")
        log.info("═" * 60)

        # Start HTTP server immediately so Railway health checks pass
        # while the rest of startup (auth, seeding) completes
        ...
        async def position_monitor():
            """
            Every 30s poll TastyTrade positions and reconcile against state.
            This frees a trade slot immediately when a position is manually closed.
            The 90s grace period on new positions (via recorded_at) prevents
            evicting a position that TastyTrade's API hasn't reflected yet.
            Does NOT hold the engine lock — reconcile_positions only removes
            positions older than 90s so it cannot race with execute_entry.
            """
            await asyncio.sleep(30)   # give startup a moment to settle
            while True:
                try:
                    if self._is_market_hours():
                        await asyncio.to_thread(self.reconcile_positions, True)
                        # Push updated counters/positions to status immediately
                        self.status.s15_trades_today = self.state.get("s15_trades_today", 0)
                        self.status.sd_trades_today  = self.state.get("sd_trades_today",  0)
                        self.status.update_positions(
                            self.state.get("s15_positions", {}),
