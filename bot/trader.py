...
async def execute_entry(client, symbol, intended_price, signal,
    ...
    # ── HARD SAFETY GATE — never place real orders in paper trading mode ──────
    if paper:
        log.info("  📋 [PAPER] Signal logged but NO order placed — paper_trading=true")
...
class AlgoEngine:
    ...
    async def run(self):
        log.info("═" * 60)
        log.info("  TastyTrade Multi-Strategy Trader  v2.6")
        log.info("═" * 60)
