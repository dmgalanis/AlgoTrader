...
class AlgoEngine:
    ...
    def reconcile_positions(self, periodic: bool = False):
        ...
        Removes any tracked position no longer open and decrements the trade
        counter so the daily slot re-opens after a manual close.
        ...
        try:
            ...
            for pos_key, counter_key in (("s15_positions", "s15_trades_today"),
                ...
                stale = [s for s in self.state.get(pos_key, {}) if s not in syms]
                for s in stale:
                    pos = self.state[pos_key][s]
                    # Don't evict a position entered in the last 90s — TastyTrade's
                    # positions API can lag after a fresh fill, and evicting early
                    # would delete the position mid-execution and skip stop/TP.
