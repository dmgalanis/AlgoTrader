...
class _StatusHandler(BaseHTTPRequestHandler):
    ...
    engine:           object          = None  # AlgoEngine ref for instant reload
    ...
    def do_POST(self):
        ...
        try:
            ...
            # Immediately reload so mode/client switches without waiting 60s
            if self.engine is not None:
                try:
                    self.engine.reload_settings()
                except Exception as re:
                    log.warning("⚠️  Immediate reload failed: %s", re)
...
def start_status_server(tracker: StatusTracker, engine=None):
    secret = os.environ.get("DASHBOARD_SECRET", "")
    _StatusHandler.tracker          = tracker
    _StatusHandler.engine           = engine
...
class AlgoEngine:
    ...
    def reload_settings(self):
        ...
        # Always sync mode directly from settings so the dashboard reflects it immediately
        new_paper = ex.get("paper_trading", True)
        self.status.mode = "PAPER" if new_paper else "LIVE"

        # If paper_trading flipped, restart the process so the correct
        # API environment (cert vs live) and credentials are loaded fresh
        if new_paper != prev_paper:
            log.info(
                "paper_trading changed to %s — saving state and restarting bot...",
    ...
    async def run(self):
        ...
        start_status_server(self.status, engine=self)
