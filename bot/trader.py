...
class TastyTradeClient:
    ...
    def poll_fill(self, order_id: str) -> Optional[dict]:
        ...
        We poll for up to FILL_POLL_TIMEOUT seconds and log every status we see
        so we can diagnose if the status string ever differs from "Filled".
        """
        ...
        while time.time() < deadline:
            try:
                ...
                if status == "Filled":
                    ...
                    # Filled but detail not populated yet — try top-level fields
                    ...
                    # TastyTrade marked Filled but hasn't populated qty yet — loop
                    # back and retry after the normal sleep interval
                    log.info("  🔄 Order %s Filled but details pending — retrying...", order_id)
        ...
        try:
            ...
            if status == "Filled":
                ...
                if filled_qty > 0:
                    return {"filled_quantity": filled_qty, "average_fill_price": avg_price}
                # Status is Filled but TastyTrade hasn't populated qty — this
                # happens on live accounts. Return qty=0 with price=0 as signal
                # to execute_entry to use the fallback unconfirmed path.
                log.warning("  ⚠️  Order %s Filled but qty still 0 after timeout", order_id)
                return None
        except Exception as e:
...
async def execute_entry(client, symbol, intended_price, signal,
    ...
    try:
        ...
        if not fill or fill["filled_quantity"] <= 0:
            # Fill not confirmed — but the buy order WAS submitted. Write the
            # position to state using intended_price so the bot does not re-enter.
            # The stop/TP won't be placed — operator must handle manually.
            log.error("  ❌ [%s] Fill not confirmed within timeout — recording position "                      "at intended price to prevent re-entry. CHECK TASTYTRADE MANUALLY.", symbol)
            risk_cfg2 = strategy_cfg["risk"]
            stop_pct2 = risk_cfg2["stop_loss_pct"] / 100
            fallback_stop = round(intended_price * (1 - stop_pct2), 2)
            fallback_qty  = shares   # shares calculated above
            state[position_key][symbol] = {
                ...
                "stop_order_id": None,
                "tp_tiers":      [],
                "fill_unconfirmed": True,
            }
            state[trades_key] = state.get(trades_key, 0) + 1
            save_state(state)
            log.error("  ⚠️  [%s] Position recorded as UNCONFIRMED — trades=%d", symbol, state[trades_key])
            return False
