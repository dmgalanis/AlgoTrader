...
class AlpacaStreamHandler:
    ...
    def _dispatch(self, msg: dict):
        ...
        Alpaca quote message fields (type "q"):
          ...
          ap — ask price
          as — ask size
          t  — RFC-3339 timestamp (we use time.time() for consistency)
        """
        if msg.get("T") != "q":
            return
        symbol = msg.get("S", "")
...
class AlgoEngine:
    ...
    async def run(self):
        log.info("═" * 60)
        log.info("  TastyTrade Multi-Strategy Trader  v1.5 (Alpaca stream)")
        log.info("═" * 60)
