...
async def execute_entry(client, symbol, intended_price, signal,
    ...
    # HARD SAFETY GATE: check paper_trading and client.paper
    # paper_trading is read from settings file; client.paper set at init
    try:
        with open(SETTINGS_PATH) as _f:
            _is_paper = json.load(_f)["execution"]["paper_trading"]
    except Exception:
        _is_paper = True  # safe default — if file unreadable, don't trade
    if _is_paper or client.paper:
        log.info("  [PAPER] No order placed — paper_trading=true")
        return False
