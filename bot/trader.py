...
async def execute_entry(client, symbol, intended_price, signal,
    ...
    # HARD SAFETY GATE: read paper_trading fresh from disk on every call
    try:
        with open(SETTINGS_PATH) as _f:
            _is_paper = json.load(_f)["execution"]["paper_trading"]
    except Exception:
        _is_paper = True  # safe default
...
def fetch_alpaca_15min_bars(symbol: str, api_key: str, secret_key: str,
    ...
    while len(bars) < count:
        # Fetch newest bars first (sort=desc) then reverse to chronological.
        # Using start+asc returns the oldest bars first and hits page limits,
        # giving stale prices from months ago rather than the most recent candles.
        params = {
            "timeframe":  "15Min",
            "limit":      limit,
            "feed":       ALPACA_FEED,
...
class AlgoEngine:
    ...
    async def _evaluate(self, symbol: str, price: float, ts: float):
        try:
          async with self._lock:
            ...
            # Read fresh from disk — never trust cached value for execution gate
            try:
    ...
    async def run(self):
        log.info("═" * 60)
        log.info("  TastyTrade Multi-Strategy Trader  v3.6")
        log.info("═" * 60)
