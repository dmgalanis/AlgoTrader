...
class _StatusHandler(BaseHTTPRequestHandler):
    tracker: "StatusTracker" = None   # injected before server starts

    def do_GET(self):
        if self.path not in ("/status.json", "/"):
            self.send_response(404); self.end_headers()
            return
        data = json.dumps(self.tracker.to_dict()).encode()
...
def start_status_server(tracker: StatusTracker):
    ...
    t.start()
    log.info("🌐 Status server running on http://0.0.0.0:%d/status.json", STATUS_PORT)
    return server
...
class TastyTradeClient:
    ...
    def get_streamer_token(self) -> tuple:
        resp = requests.get(f"{self.base}/quote-streamer-tokens", headers=self._h(), timeout=10)
        resp.raise_for_status()
        d = resp.json()["data"]
...
class AlgoEngine:
    ...
    async def _evaluate(self, symbol: str, price: float, ts: float):
        async with self._lock:
            ...
            # Keep status positions + equity fresh
    ...
    async def run(self):
        log.info("═" * 60)
        log.info("  TastyTrade Multi-Strategy Trader  v1.3")
        log.info("═" * 60)
