...
def fetch_alpaca_15min_bars(symbol: str, api_key: str, secret_key: str,
    ...
    """
    Fetch up to  historical 15-minute bars for  from Alpaca.
    Returns a DataFrame with columns: ts (unix epoch float), close, open, high,
    low, volume — matching CandleBuilder's internal format exactly so bars can
    be loaded directly into history.

    Uses the /v2/stocks/{symbol}/bars endpoint with timeframe=15Min.
    Handles pagination automatically if count > 1000 (Alpaca's page limit).
    """
    headers = {
    ...
    # Alpaca bar fields: t (ISO timestamp), o, h, l, c, v
    df["ts"]     = pd.to_datetime(df["t"]).astype("int64") // 10**9   # unix seconds float
...
class AlgoEngine:
    ...
    def _seed_15min_candles(self, symbols: list):
        ...
        for sym in symbols:
            ...
            if df.empty:
                ...
                continue
            # Load into CandleBuilder history — skip the last bar since it may
            # be the still-open current candle; live ticks will handle that.
            for row in df.iloc[:-1].itertuples(index=False):
                ...
                self.candles_15m.history[sym].append(bar)
            # Seed _current with the last bar so the candle boundary logic
            # works correctly when the first live tick arrives.
            last = df.iloc[-1]
            ...
            log.info("  ✅ %s seeded with %d completed bars.", sym, len(df) - 1)
    ...
    async def run(self):
        log.info("═" * 60)
        log.info("  TastyTrade Multi-Strategy Trader  v1.9")
        log.info("═" * 60)
