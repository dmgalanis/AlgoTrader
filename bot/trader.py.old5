"""
TastyTrade Multi-Strategy Auto-Trader v1.5
==========================================
New in v1.5:
  - GET  /paper-ledger.json  — serves paper trading ledger to dashboard
  - POST /paper-ledger       — dashboard pushes ledger updates (syncs across devices)
  - paper_ledger.json persisted to config/ so ledger survives restarts
...
PAPER_LEDGER_PATH = os.path.join(ROOT, "config", "paper_ledger.json")
...
class _StatusHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        elif self.path == "/paper-ledger.json":
            try:
    ...
    def do_POST(self):
        if self.path == "/paper-ledger":
            self._handle_paper_ledger_post()
    ...
    def _check_secret(self) -> bool:
        """Returns True if auth passes (or no secret configured)."""
        secret = self.dashboard_secret
        if not secret:
            return True
        provided = self.headers.get("X-Dashboard-Secret", "")
        if provided != secret:
            log.warning("⚠️  Request rejected — invalid secret from %s",
    ...
    def _read_json_body(self):
        """Read and parse the request body as JSON. Returns (data, None) or (None, error_str)."""
    ...
    def _handle_paper_ledger_post(self):
        ...
        try:
            ...
            log.error("❌ Failed to write paper_ledger.json: %s", e)
            self._send_json(500, {"error": str(e)})
