...
class TastyTradeClient:
    ...
    def place_limit(self, symbol: str, quantity: int, limit_price: float) -> str:
        order = {
            ...
            "price": str(round(limit_price, 2)), "price-effect": "Credit",
            "legs": [{"instrument-type": "Equity", "symbol": symbol,
                      "quantity": quantity, "action": "Sell to Close"}],
        }
        resp = requests.post(
...
class AlgoEngine:
    ...
    async def run(self):
        ...
        async def position_monitor():
            ...
            so the bot can re-enter on the next signal rather than waiting for restart.
            Only runs during market hours to avoid unnecessary API calls overnight.
            """
