...
def fetch_alpaca_15min_bars(symbol: str, api_key: str, secret_key: str,
    ...
    while len(bars) < count:
        # Go back 30 calendar days to ensure we get enough bars.
        # Without an explicit start date Alpaca may only return the current session.
        ...
        params = {
            ...
            "feed":       ALPACA_FEED,
            "sort":       "asc",
            "adjustment": "raw",   # no split/dividend adjustment for intraday
        }
        if cursor:
...
class AlgoEngine:
    ...
    async def run(self):
        log.info("═" * 60)
        log.info("  TastyTrade Multi-Strategy Trader  v2.1")
        log.info("═" * 60)
