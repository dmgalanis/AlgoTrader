...
class _StatusHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        # Always update — including when turning OFF — so the dashboard toggle
        # actually stops trading immediately.
        if not algo_requested:
            # Turning off — always allow, no confirmation needed
            ...
            # Turning on — only allow if paper mode or live confirmed
...
async def execute_entry(client, symbol, intended_price, signal,
    ...
    try:
        ...
        # ── Update state IMMEDIATELY after confirmed fill ─────────────────
        # This must happen before place_stop / place_limit so that if either
        # throws, the position is still tracked and the bot won't re-enter.
        state[position_key][symbol] = {
            ...
            "buy_order_id":  buy_id,
            "stop_order_id": None,   # filled in below once placed
            "tp_tiers":      [],     # filled in below once placed
        }
        ...
        try:
            ...
            log.error("  ❌ Stop-loss placement failed for %s: %s — position tracked, manual stop needed", symbol, e)

        # ── Place take-profit tiers ───────────────────────────────────────
        ...
        for tier in strategy_cfg["take_profit_tiers"]:
            ...
            try:
                ...
                log.error("  ❌ TP tier %sR placement failed for %s: %s", tier["r_multiple"], symbol, e)
