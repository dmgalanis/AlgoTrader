"""
TastyTrade Multi-Strategy Auto-Trader v4.3
==========================================
...
def start_status_server(tracker: StatusTracker):
    ...
    t.start()
    log.info("🌐 Status server on http://0.0.0.0:%d  (PORT env=%s, secret: %s)",
             STATUS_PORT, os.environ.get("PORT", "not set"), "set ✅" if secret else "not set ⚠️")
    return server
...
class AlgoEngine:
    ...
    async def run(self):
        log.info("═" * 60)
        log.info("  TastyTrade Multi-Strategy Trader  v4.3")
        log.info("═" * 60)
