...
class TastyTradeClient:
    ...
    def place_oco(self, symbol: str, stop_qty: int, tp_qty: int,
        ...
        resp.raise_for_status()
        data = resp.json()["data"]
        log.debug("  🔍 OCO response keys: %s", list(data.keys()))
        # TastyTrade returns either data["id"] or data["order"]["id"] depending on version
        oco_id = data.get("id") or data.get("order", {}).get("id") or data.get("complex-order-id", "")
        if not oco_id:
            log.warning("  ⚠️  OCO id not found in response — keys: %s", list(data.keys()))
            oco_id = str(data)[:50]
        return str(oco_id)
    ...
    def get_daily_candles(self, symbol: str, count: int = 100) -> pd.DataFrame:
        # TastyTrade candles endpoint uses 'period-type' and 'period' not 'timeframe'
        # Try the correct parameter format; fall back to Alpaca if needed
        resp = requests.get(
            f"{self.base}/market-data/candles/{symbol}",
            headers=self._h(),
            params={"period-type": "day", "period": count},
            timeout=15)
        if resp.status_code == 400:
            # Some symbols use 'timeframe' format instead
            resp = requests.get(
                f"{self.base}/market-data/candles/{symbol}",
                headers=self._h(),
                params={"timeframe": "1d", "period": count},
                timeout=15)
        resp.raise_for_status()
        raw = resp.json()
        log.info("  📊 Daily candles for %s — raw keys: %s", symbol, list(raw.get("data", {}).keys()))
        df = pd.DataFrame(raw["data"]["items"])
        log.info("  📊 %s columns: %s  rows: %d", symbol, list(df.columns), len(df))
        # Normalise column names — TastyTrade may return closePrice / adjClose / close
        col_map = {}
        for col in df.columns:
            lc = col.lower()
            if lc in ("closeprice", "adjclose", "adj_close", "close-price"):
                col_map[col] = "close"
            elif lc in ("opentime", "datetime", "open-time"):
                col_map[col] = "time"
        if col_map:
            log.info("  🔄 Renaming columns: %s", col_map)
...
async def execute_entry(client, symbol, intended_price, signal,
    ...
    try:
        ...
        if fill_price == intended_price and fill.get("average_fill_price") == 0.0:
            log.info("  ℹ️  Fill price unavailable — using intended price $%.4f for stop/TP calc", intended_price)
